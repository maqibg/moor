...
- 785fd52: 优化 MCP Gateway 性能：允许同一 Server 并发工具调用，缓存工具目录与设置，异步批量写入并按保留期清理审计日志；日志页面改为防抖筛选和分页加载。

## 0.6.4

### Patch Changes

- 67e0e0d: 完善 MCP Streamable HTTP 网关：新增 GET SSE、Session 生命周期、协议版本协商和工具目录变更通知；端口 fallback 后保存实际端口并提示更新客户端配置。

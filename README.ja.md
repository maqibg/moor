...
`9223` はデフォルトの Sidecar ポートです。既に使用されている場合、Moor は次に利用可能なポートを選択して次回以降の起動用に保存し、実際のエンドポイントを警告表示します。この場合は既存の MCP クライアント設定を更新してください。
...
| メソッド | パス   | 説明                                     |
| -------- | ------ | ---------------------------------------- |
| `POST`   | `/mcp` | MCP JSON-RPC リクエストと通知を送信      |
| `GET`    | `/mcp` | Streamable HTTP SSE 通知ストリームを開く |
| `DELETE` | `/mcp` | MCP Session を終了                       |
